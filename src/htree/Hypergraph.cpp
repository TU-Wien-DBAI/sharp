...
/*
***Description***
The method creates the internal representation of a hypergraph given by the parser.

INPUT:	P: Pointer to a parser object
OUTPUT: 
...
void Hypergraph::resetEdgeLabels()
{
	for(int i=0; i < iMyMaxNbrOfEdges; i++)
		MyEdges[i]->setLabel(0);
}


// Sets labels of all nodes to zero
void Hypergraph::resetNodeLabels()
{
	for(int i=0; i < iMyMaxNbrOfNodes; i++)
		MyNodes[i]->setLabel(0);
}
